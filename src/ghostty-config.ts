...
export function parseGhosttyConfig(overridePaths?: string[]): GhosttyConfig {
    ...
    for (const line of lines) {
        ...
        const withoutComment = commentIdx !== -1 ? value.slice(0, commentIdx).trim() : value;

        // Strip surrounding double quotes (e.g. theme = "Ayu Light")
        const cleanValue = withoutComment.startsWith('"') && withoutComment.endsWith('"')
    ...
    for (const themePath of candidates) {
        try {
            ...
            for (const line of content.split('\n')) {
                ...
                const withoutComment = commentIdx !== -1 ? value.slice(0, commentIdx).trim() : value;
                const cleanValue = withoutComment.startsWith('"') && withoutComment.endsWith('"')
                    ? withoutComment.slice(1, -1) : withoutComment;
